...
<div class="section-card">
  <h3>2. Stored XSS <span class="badge-ok">(Safe)</span></h3>
  <p><b>Title:</b> <?= $post ? h($post['title']) : 'Not found' ?></p>
  <div class="field">
    <label>Post content (encoded)</label>
    <pre><?= $post ? h($post['content']) : '' ?></pre>
  </div>
  <h4>Comments (encoded)</h4>
  <ul>
    <?php foreach ($comments as $cm): ?>
      <li><b><?= h($cm['author']) ?>:</b> <span><?= h($cm['body']) ?></span></li>
    <?php endforeach; ?>
  </ul>
  <h4>Add Comment</h4>
...
<?php layout('Stored XSS (safe)', ob_get_clean());